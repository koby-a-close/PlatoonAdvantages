...
"""Looks at platoon advantages from HR/FB Rates. New metric based on research in The Book."""
...
df_qualified_batters = df_qualified_batters[['Name', 'HR/FB']]
df_qualified_batters['Tag'] = np.nan
df_qualified_batters.Tag[(df_qualified_batters['HR/FB'] > 0.18)] = 'High'
df_qualified_batters.Tag[(df_qualified_batters['HR/FB'] < 0.11)] = 'Low'
...
df_qualified_pitchers = df_qualified_pitchers[['Name', 'HR/FB']]
df_qualified_pitchers['Tag'] = np.nan
df_qualified_pitchers.Tag[(df_qualified_pitchers['HR/FB'] > 0.16)] = 'High'
df_qualified_pitchers.Tag[(df_qualified_pitchers['HR/FB'] < 0.12)] = 'Low'
...
t = PrettyTable(['Batter | Pitcher', 'High', 'Neutral', 'Low'])
t.add_row(['High', H_H_wOBA, N_H_wOBA, L_H_wOBA])
t.add_row(['Neutral', H_N_wOBA, N_N_wOBA, L_N_wOBA])
t.add_row(['Low', H_L_wOBA, N_L_wOBA, L_L_wOBA])
print(t)

data = t.get_string()
with open('HR-FBRate.txt', 'w') as info:
    info.write(data)
